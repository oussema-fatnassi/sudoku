...
void Grid::loadGridFromFile(const char* filename, int gridIndex) {
    ...
    if (gridIndex < 0 || gridIndex >= gridCount) {
        std::cerr << "Error: Invalid grid index" << std::endl;
        return;
    }

    cells = allGrids[gridIndex]; // Load the specified grid
    originalRandomIndex = gridIndex;
    copyUnsolvedGrid();
}
...
void Grid::chooseDifficulty(){                                                      // Method to choose the difficulty level
    ...
    switch (choice) {                                                               // Load the grid based on the difficulty level
        case 1:
            loadGridFromFile("../assets/easy.txt",0);
            difficulty = 1;
            break;
        case 2:
            loadGridFromFile("../assets/medium.txt",0);
            difficulty = 2;
            break;
        case 3:
            loadGridFromFile("../assets/hard.txt",0);
            difficulty = 3;
            break;
        default:
            cout << "Invalid choice. Please enter a number between 1 and 3." << endl;
